  ...
  const reportWindowSize = () => {
    ...
    const { 
        ...
        items: featureFilter.map( (i, index) => 
            ...
            id: `item-${index}`,
            renderItem: (
              <div onClick={goToProduct} data-idlink={`${i.node.productCategories.nodes[0].slug}/${i.node.slug}`} className='heroproduct-render'>
...
return (
    <>
        ...
        <button className='heroprev' onClick={slideToPrevItem}></button>
        {carouselFragment}
        <button className='heronext' onClick={slideToNextItem}></button>
      </div>
    </>
)
