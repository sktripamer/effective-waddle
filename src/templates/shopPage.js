...
const restartAnimation = () => {
    ...
    return (
        ...
        <div onClick={changePage} data-idlink={`${item.node.productCategories.nodes[0].slug}/${item.node.slug}`} class={`mini-product-cont ${item.node.productTags.edges.map(tags => slugify(tags.node.name) ).join('')}`}>
