...
export default function Verify() {
    ...
    async function handleVerify() {
      ...
      async function handlePassword() {
        ...
        const request = await fetch('/api/verify-pwdpw', {
            method: 'POST',
      ...
      async function verifyCode(req) {
          if (isBrowser) {
        const request = await fetch('/api/verify-codepw', {
            method: 'POST',
            body: req,
          });
          const intent = (await request.json());
                ...
                ? (
                 <div className='search-outer-cont'>
                   ...
                   <div class='email-sent'>
                     <div class="verify-email-text">{`Please click the link that we sent to ${emailText} to change your password.`}</div>
                   <div id="anim-wrapper">
  <div id="anim-bg">
    <div id="env-wrapper">
...
<a class='goto-email' href={`https://${goToEmail}`} target="_blank" rel="noreferrer noopener">
  {`Go To ${goToEmail.substr(0, goToEmail.lastIndexOf("."))}`}
    ...
    <div class='email-to-use'>{emailParam}</div>
        <div class='set-your-password'>Create your new password to continue:</div>
